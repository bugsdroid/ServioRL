...
class SubtitleRepository {
  ...
  Future<List<SubtitleItem>> getMissingEpisodes() async {
    final res  = await _dio.get('/api/episodes',
        queryParameters: {'start': 0, 'length': 100});
    final data = res.data as Map<String, dynamic>;
    final list = data['data'] as List<dynamic>? ?? [];
    return list
  ...
  Future<List<SubtitleResult>> searchMovieSubtitles({
    ...
    final res  = await _dio.get('/api/providers/movies',
        queryParameters: {'radarrid': radarrId, 'language': language});
    final data = res.data as Map<String, dynamic>;
    final list = data['data'] as List<dynamic>? ?? [];
    return list
  ...
  Future<List<SubtitleResult>> searchEpisodeSubtitles({
    ...
    final res  = await _dio.get('/api/providers/episodes',
        queryParameters: {'episodeid': episodeId, 'language': language});
    final data = res.data as Map<String, dynamic>;
    final list = data['data'] as List<dynamic>? ?? [];
    return list
        .map((j) => SubtitleResult.fromJson(j as Map<String, dynamic>))
  ...
  Future<void> autoSearchMovie(String radarrId) async {
    await _dio.post('/api/movies',
        data: {'radarrid': radarrId, 'hi': 'False', 'forced': 'False'});
  }

  Future<void> autoSearchEpisode(String episodeId) async {
    await _dio.post('/api/episodes',
        data: {'episodeid': episodeId, 'hi': 'False', 'forced': 'False'});
  }
}
...
class MissingSubtitlesNotifier extends AsyncNotifier<List<SubtitleItem>> {
  @override
  Future<List<SubtitleItem>> build() async {
    final cfg = ref.read(appConfigProvider);
    if (cfg.bazarrBaseUrl.isEmpty || cfg.bazarrApiKey.isEmpty) return [];
    return _fetch();
  }
  ...
  Future<void> refresh() async {
    final cfg = ref.read(appConfigProvider);
    if (cfg.bazarrBaseUrl.isEmpty || cfg.bazarrApiKey.isEmpty) {
